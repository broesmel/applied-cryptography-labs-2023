...
def main():
    ...
    for b in range(len_flag_blocks):
        ...
        while low <= high:
            ...
            # if we errored, our guess is too small
            if not res:
                low = last_mm_i + 1
    
            # If we are large enough, our guess is correct or too large
            else:
                # test if too large
                high = last_mm_i-1
                ctxt_mod = ctxt_blocks[0] + c11_mod
                ctxt_mod += bytes([0]*16)*high
                try:
                    tmp_last_byte = bytes([high ^ ctxt_blocks[prev_i][-1] ^ ctxt_blocks[0][-1]])
                except ValueError as e:
                    print('xor failed', e)
                    continue

                print(f"{high=}=>{tmp_last_byte}", 
                    r2 := snd_rcv({
                        "command": "metadata_leak", 
                        "ctxt": ctxt_mod.hex(), "m0": m0.hex(), "c0": (c0).hex()
                    }), end='\r'
                )
                # one smaller still large enough, go again
                if r2.get('metadata', False):
                    continue
